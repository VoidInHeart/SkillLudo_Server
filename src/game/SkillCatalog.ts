import type { PlayerColor, PlayerSkillState } from '../protocol.js';
...
export const SKILL_CATALOG: readonly SkillDescription[] = [
  ...
  { id: 'cn-roar', color: 'BLUE', name: '雄狮之吼', kind: 'AWAKENING', description: '所有玩家投出的两枚原始骰点累计超过 100 后，永久解锁「尺有所长」。技能改点不计入累计。' },
  { id: 'cn-scale', color: 'BLUE', name: '尺有所长', kind: 'COOLDOWN', description: '觉醒后，预选点数可 +1 或 -1，冷却 3 个己方正常回合。下一正常回合首次行动必须反向调整同样幅度。每个正常回合最多使用一次，改点不触发连投；有效步数最低为 0。' },
  { id: 'cn-grit', color: 'BLUE', name: '坚韧不拔', kind: 'NORMAL', description: '被击落后直接返回起飞处，并获得 1 点能量，上限 3。每消耗 3 点强化已觉醒的「尺有所长」：第一重免除强制反向；第二重连续三轮未使用可储备一次免冷却技能，最多存一次；第三重调整范围扩大为 -2 至 +2。' },
...
/** Descriptions reflect actual upgrades, so the catalog never advertises an old range/CD. */
export function describeSkill(skillId: string, state?: Pick<PlayerSkillState, 'level'>): string {
  const base = SKILL_CATALOG.find((skill) => skill.id === skillId)?.description ?? '';
  if (!state || skillId !== 'cn-scale') return base;
  const level = state.level ?? 0;
  return `预选点数可${level >= 3 ? '调整 -2、-1、+1 或 +2' : ' +1 或 -1'}，冷却 3 个己方正常回合。`
    + (level >= 1 ? '已免除下一轮强制反向。' : '下一正常回合首次行动必须反向调整同样幅度。')
    + (level >= 2 ? '连续三轮未使用时储备一次：可在冷却中使用，不启动或改变冷却，最多储备一次。' : '')
    + '每个正常回合最多使用一次（含储备），连投不刷新使用次数。改点不触发连投，最低 0 步。';
}

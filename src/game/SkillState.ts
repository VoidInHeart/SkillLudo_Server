...
export function refreshAwakening(room: Room): void {
  ...
  for (const player of room.players) {
    ...
    if (player.color === 'BLUE' && (game.rolledTotal ?? 0) > 50) state.awakened = true;
  }
}
...
export function actionSpecs(room: Room): ActionSpec[] {
  ...
  game.diceChoices.forEach((raw, dieIndex) => {
    const add = (id: string, dice: number, kind: ActionSpec['kind'], label: string, delta?: number, mandatory = false) => {
      if (dice >= 0 && dice <= 12) specs.push({ id, dieIndex, dice, kind, label, delta, mandatory,
        extraTurn: dice === 6 && kind !== 'UK_SUM' && kind !== 'FR_RESCUE' });
    };
    ...
    if (player.color === 'BLUE' && state.awakened && state.readyAtTurn <= state.normalTurns) {
      const range = state.level >= 2 ? [-2, -1, 1, 2] : [-1, 1];
      range.forEach((delta) => add(`cn-${dieIndex}-${delta}`, raw + delta, 'CN_SHIFT', `${raw} ${signed(delta)} = ${raw + delta}`, delta));
    }
  });
...
export function consumeAction(room: Room, option: ActionOption): void {
  ...
  if (option.mandatory) state.forcedDelta = 0;
    ...
    state.readyAtTurn = state.normalTurns + (state.level >= 3 ? 2 : 3);
  ...
  return room.players.flatMap((player) => {
    ...
    return SKILL_CATALOG.filter((skill) => skill.color === player.color).map((skill) => {
      const available = manual && (skill.id === 'uk-sun' ? game.phase === 'WAIT_SELECT_DIE' && !state.limitedUsed && !game.rescue && (game.diceChoices?.reduce((a, b) => a + b, 0) ?? 0) >= 10
        ...
        : skill.id === 'cn-scale' ? game.phase === 'WAIT_SELECT_DIE' && state.awakened && state.readyAtTurn <= state.normalTurns && !state.forcedDelta
